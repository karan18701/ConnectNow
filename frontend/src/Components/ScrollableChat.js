...
import { useToast, Text, Divider } from "@chakra-ui/react";
...
const ScrollableChat = ({ messages }) => {
  ...
  let chatDate =
    messages.length !== 0 ? getMsgDate(messages[0]?.time) : getMsgDate(null);
        ...
        messages.map((m, i) => (
          <div key={m._id}>
            <Text
              fontWeight={"hairline"}
              display={
                chatDate != getMsgDate(m?.time) &&
                changeDate(getMsgDate(m?.time))
                  ? "flex"
                  : "none"
              }
            ...
            <div style={{ display: "flex" }} key={m._id}>
              ...
              {m.type == "text" ? (
                ...
                >
                  <span style={{ fontSize: "10px" }} key={m?._id}>
                    {actualDate(m?.time)}
                  </span>
                  <br />
